...
class MixedPrecisionOptimizer:
    ...
    def get_autocast_context(self):
        ...
        return (
            torch.amp.nullcontext()
            if hasattr(torch.amp, "nullcontext")
            else _NullContext()
        )
...
def enable_gradient_checkpointing_advanced(
    model: nn.Module, checkpoint_segments: int = 1
) -> None:
    ...
    if hasattr(model, "gradient_checkpointing_enable"):
        ...
        logger.info(
            f"✓ Gradient checkpointing enabled (segments: {checkpoint_segments})"
        )
...
class DynamicTokenPruning:
    ...
    def get_pruning_mask(self, importance: torch.Tensor) -> torch.Tensor:
        ...
        threshold = torch.quantile(importance, q=self.pruning_ratio)
        mask = (importance >= threshold).float()
        return mask

    def apply_pruning(
        self, hidden_states: torch.Tensor, pruning_mask: torch.Tensor
    ) -> torch.Tensor:
