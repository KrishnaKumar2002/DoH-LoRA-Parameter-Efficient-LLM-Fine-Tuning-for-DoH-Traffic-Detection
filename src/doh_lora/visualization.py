...
def plot_training_curves(
    ...
    fig.suptitle(f"{task_name} - Training Progress & Metrics", fontsize=16, fontweight="bold")

    # 1. Training Loss
    ax = axes[0, 0]
    if history["epochs"] and history["train_loss"]:
        ax.plot(history["epochs"], history["train_loss"], "b-o", linewidth=2, markersize=4)
    ...
    if history["epochs"] and history["train_lr"]:
        ax.plot(history["epochs"], history["train_lr"], "g-s", linewidth=2, markersize=4)
        ax.set_xlabel("Epoch", fontsize=10)
    ...
    if history["epochs"] and history["accuracy"]:
        ln1 = ax.plot(history["epochs"], history["accuracy"], "r-^", linewidth=2, markersize=4, label="Accuracy")
        ax.set_xlabel("Epoch", fontsize=10)
    ...
    if history["epochs"] and history["f1_scores"]:
        ln2 = twin.plot(history["epochs"], history["f1_scores"], "m-d", linewidth=2, markersize=4, label="F1 Score")
    ...
    if history["epochs"] and history["inference_time"]:
        ln1 = ax.plot(history["epochs"], np.array(history["inference_time"]) * 1000, "c-o", linewidth=2, markersize=4, label="Inference Time")
        ax.set_xlabel("Epoch", fontsize=10)
    ...
    if history["epochs"] and history["memory_mb"]:
        ln2 = twin.plot(history["epochs"], history["memory_mb"], "orange", marker="D", linewidth=2, markersize=4, label="Memory Usage")
...
def plot_parameter_efficiency(
    ...
    fig.suptitle(f"{task_name} - Parameter Efficiency & Compression", fontsize=14, fontweight="bold")
    ...
    ax.pie(sizes, explode=explode, labels=labels, colors=colors, autopct="", shadow=True, startangle=90)
    ax.set_title(f"LoRA (r={lora_r}): Parameter Split", fontweight="bold", fontsize=11)

    # 2. Compression comparison
    ax = axes[1]
    compressions = ["Full Model\n(baseline)", "LoRA Adapter", ]
    ...
    bars = ax.bar(compressions, sizes_mb, color=colors_comp, alpha=0.8, edgecolor="black", linewidth=1.5)
...
def plot_turboquant_compression_stats(
    ...
    fig.suptitle(f"{task_name} - TurboQuant Compression Analysis", fontsize=14, fontweight="bold")
    ...
    bars = ax.bar(labels, sizes, color=colors, alpha=0.8, edgecolor="black", linewidth=1.5)
    ...
    ax.text(0.5, max(sizes) * 0.5, f"×{compression_ratio:.2f}x compression",
    ...
    bars = ax.bar(stages, bit_allocation, color=colors_stages, alpha=0.8, edgecolor="black", linewidth=1.5)
